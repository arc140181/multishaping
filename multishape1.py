...
def pha_multi(x, thr, debug=0, perc=0.1):
    ...
    colorpal = ['slateblue', 'red', 'limegreen', 'darkorange']
...
for j in range(len(NAMPL)):
    ...
    sns.histplot(height_a, element="poly", binrange=RANGE, bins=BINS, alpha=ALPHA, ax=ax[j//2,j%2], kde=False)
    sns.histplot(height_b, element="poly", binrange=RANGE, bins=BINS, alpha=ALPHA, ax=ax[j//2,j%2], kde=False)
    sns.histplot(height_c, element="poly", binrange=RANGE, bins=BINS, alpha=ALPHA, ax=ax[j//2,j%2], kde=False)
    
    pa = len(height_a) * 100 / lhast
    pb = len(height_b) * 100 / lhast
    pc = len(height_c) * 100 / lhast
    
    σa = np.std(height_a[(height_a>RANGE[0]) & (height_a<RANGE[1])])
    σb = np.std(height_b[(height_b>RANGE[0]) & (height_b<RANGE[1])])
    σc = np.std(height_c[(height_c>RANGE[0]) & (height_c<RANGE[1])])
    
    ax[j//2,j%2].legend(["Narrow\nσ={:.3f}\n{:.2f}%".format(σa, pa), "Multi\nσ={:.3f}\n{:.2f}%".format(σb, pb), "Wide\nσ={:.3f}\n{:.2f}%".format(σc, pc)], frameon=False)
    ...
    ax[j//2,j%2].text(0.05, 0.95, 'N={:.2f}\nf={:.2f}'.format(NAMPL[j], FREQ), transform=ax[j//2,j%2].transAxes, fontsize=10, verticalalignment='top')
    ...
    for i in range(len(count)):
        bx[j].bar(' ', count[i], 1.1, bottom, color=colorpal[i], alpha=ALPHA*2)
        bottom += count[i]
        bx[j].text(-0.48, -3+sum(count[0:i])+count[i]/2, '{:.1f}%'.format(count[i]))
